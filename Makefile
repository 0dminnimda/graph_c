CC = "$(shell (which clang 2>/dev/null || which gcc 2>/dev/null) | head -1)"
PYTHON = "$(shell (which python3 2>/dev/null || which python 2>/dev/null) | head -1)"
O = 1

FLAGS := -pedantic -Wall -Wextra -Werror
FLAGS := $(FLAGS) -std=c11

ifeq ($(DEBUG),2)
	FLAGS := $(FLAGS) -gdwarf-4
else ifeq ($(DEBUG),1)
	FLAGS := $(FLAGS) -g
endif

ifeq ($(SAN),2)
	# address is incompatable with memory
	FLAGS := $(FLAGS) -fsanitize=memory,undefined
else ifeq ($(SAN),1)
	FLAGS := $(FLAGS) -fsanitize=address
endif

BASIC = src/basic/str.c src/basic/str_builder.c src/basic/read_line.c \
		src/basic/str_to_int.c src/basic/type_operations.c
SRC = src/main.c src/graph.c src/names.c

.PHONY: build
build:
	$(CC) -o main.out $(WARN) -O$(O) $(SRC) $(BASIC) $(FLAGS)

.PHONY: record
record:
	./record_tests.sh ./main.out --no-glow

.PHONY: tests
tests:
	./check_tests.sh ./main.out --no-glow
	# ./check_tests.sh valgrind --leak-check=full ./main.out --no-glow

.PHONY: clean
clean:
	rm ./main.out

